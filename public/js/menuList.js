        ...
        success: (res) => {
            ...
            for (let i = 0; i < allMenus.length; i++) {
                ...
                if (adminPosition != "guest") {
                    ...
                    tempRow.push(
                        `<img width="37px" src="${allMenus[i].menuUrl}">`
                    );
                    ...
                    tempRow.push(allMenus[i].menuRecommendCnt.toString());
                    tempRow.push(allMenus[i].menuLikeCnt.toString());
                    tempRow.push(`<input type="button" id="${allMenus[i].menu_id}" onClick="alert('권한이 필요한 작업입니다!')"
                    class="btn btn-outline-primary" value="상세페이지">`);
                    tempRow.push(
                        `<img style="cursor:pointer;" src="../public/assets/img/deleteBtn.png" id="${allMenus[i].menu_id}" onClick="alert('권한이 필요한 작업입니다!')" width="40px">`
                    );
                }
